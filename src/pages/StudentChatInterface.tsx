...
import {
    safeParseLogicFlow,
    hasMacroRefinement,
    getMacroOrderedLabels,
    getMacroBlocksDescription,
} from '../lib/logic-flow';
...
export default function StudentChatInterface() {
    ...
    const getContextString = (q: Question) => {
        ...
        const macroRefine = hasMacroRefinement(logicFlowParts);
        const macroLabels = getMacroOrderedLabels(logicFlowParts);
        const macroBlockText = macroRefine
            ? `\n\n[교사 논리 흐름 — 큰 범주(의미 단위)]\n${getMacroBlocksDescription(logicFlowParts)}\n큰 범주 개수: ${macroLabels.length}\n큰 범주 순서: ${macroLabels.map(l => `[${l}]`).join(' → ')}\n※ 추천 질문 Step 1(「의미 기준으로 몇 부분」): 1~2번째 클릭 시에는 큰 범주 기준으로만 유도하고, 3~4번째 클릭부터는 아래 세부 부분 수·작은 범주 라벨을 정답 기준으로 하세요. 그 외 Step 2·3 및 일반 질문은 항상 세부 구조를 따르세요.\n`
            : '';
        const logicFlowInfo = logicFlowParts.length > 0
            ? `\n\n★ 교사 논리 흐름 데이터 — 세부 부분 (총 ${logicFlowParts.length}부분, 세부 정답) ★\n${logicFlowParts.map((step, i) => {
                const macro = step.macroLabel?.trim() || '미입력';
                const small = step.role?.trim();
                const smallPart = small ? ` 작은 범주 [${small}]` : '';
                return `부분 ${i + 1}: 큰 범주 [${macro}]${smallPart} ${step.content}${step.conjunction ? ` (연결어: ${step.conjunction})` : ''}`;
            }).join('\n')}`
            : '';
        const topicInfo = q.topic ? `\n우리말 주제: ${q.topic}` : '';
        const structureGuide = logicFlowParts.length > 0
            ? `\n\n[구조 강조 지침]\n최종·세부 정답은 위 세부 부분 ${logicFlowParts.length}개, 각 행의 큰 범주·작은 범주(있는 경우)·중심 의미입니다.${macroRefine ? ' 단, 추천 질문 Step 1의 앞 두 번(1~2단계 힌트)에 한해 큰 범주 기준으로 유도할 수 있습니다.' : ''} 학생에게는 상황에 맞게 직접 정답을 말하지 말고, 네 자체 분석으로 부분 수를 바꾸지 마세요.`
            : '';
        return `[문항 정보]\n문항 번호: ${q.examCode}\n출처: ${q.targetGrade || '-'}\n지문: ${q.passage}${topicInfo}${macroBlockText}${logicFlowInfo}${structureGuide}`;
    };
        ...
        try {
            const systemMessage = {
                ...
                content: (roomInfo.system_prompt || '너는 친절하고 꼼꼼한 선생님이야.') +
                    ...
                    `\n2. 특히 '논리 흐름'의 부분 수와 각 부분의 큰 범주·작은 범주(있는 경우) 라벨을 절대 변경하거나 생략하지 마세요.` +
                    `\n3. 답변 시 교사가 정한 라벨 형식을 반드시 그대로 사용하세요.` +
                    `\n\n${contextString}`
            };
    ...
    const handleFAQClick = async (index: number) => {
        ...
        if (faqEntry.isSentenceModal) {
            ...
            if (currentCount === 1) {
                ...
                repeatNotice = `\n\n[힌트 수준: 4단계 - 정답 공개 + 칭찬] (${currentCount}번째 질문)\n학생이 여러 번 노력했어. 먼저 학생의 끈기와 노력을 진심으로 칭찬해줘. 그 다음 교사 데이터의 정답을 교사 데이터의 형식 그대로 학생에게 보여줘. 교사 데이터에 있는 큰 범주·작은 범주·중심 의미를 그대로 사용해서 정리해줘. 정답을 설명한 후, "잘 했어! 이제 다음 질문으로 넘어가볼까?" 라고 다른 질문을 시도하도록 자연스럽게 유도해.`;
            }
            ...
            // Step 1: initial part count — Socratic tutoring (1~2회: 큰 범주, 3~4회: 세부 — 큰 범주가 설정된 경우에만)
            else if (questionText === FLAT_FAQ[1]?.text && selectedQuestion.logicFlow) {
                const macroLayerActive = hasMacroRefinement(logicFlowParts);
                const macroLabels = getMacroOrderedLabels(logicFlowParts);
                const macroCount = macroLabels.length;
                const detailCount = logicFlowParts.length;
                const useMacroPhase = macroLayerActive && currentCount <= 2;
                const strictCount = useMacroPhase ? macroCount : detailCount;

                let step1LevelGuide = '';
                if (useMacroPhase) {
                    if (currentCount === 1) {
                        step1LevelGuide = `\n1단계 안내 (큰 범주 기준): 글의 의미 흐름이 크게 바뀌는 지점을 떠올리게만 유도해. 아직 '몇 부분'이라는 숫자, 큰 범주 이름, 작은 범주 라벨(예: 설명1)은 직접 말하지 마. (부분이 없다고 말하는 것은 금지, 나눌 수 있음을 암시)`;
                    } else {
                        step1LevelGuide = `\n2단계 안내 (큰 범주 기준): 지문을 큰 의미 덩어리로 보면 대략 ${macroCount}개 전후의 흐름으로 나눌 수 있다는 뉘앙스만 줄여. 작은 범주 라벨은 아직 말하지 마.`;
                    }
                } else {
                    if (currentCount === 1) {
                        ...
                        step1LevelGuide = `\n3단계 안내: "이 글은 ${detailCount - 1}부분일까, 아니면 ${detailCount}부분일까?" 처럼 정답에 근접한 보기를 줘.`;
                    } else {
                        const actualLabels = logicFlowParts.map(s => {
                            const sm = s.role?.trim();
                            if (sm) return `[${sm}]`;
                            const mac = s.macroLabel?.trim();
                            return mac ? `[${mac}]` : '[미지정]';
                        }).join(', ');
                        step1LevelGuide = `\n4단계 안내: 학생의 노력을 칭찬해줘. 그리고 정답으로 정확한 부분 수(${detailCount}개)와 각 부분을 식별하는 라벨(${actualLabels})만 알려줘. (작은 범주가 비어 있으면 큰 범주 라벨을 쓰면 됨)\n★★ 절대 금지: 각 부분의 중심 내용(content)은 이 단계에서 절대 알려주지 마. 중심 내용은 다음 질문(Step 2)에서 다룰 거야.\n"잘 했어! 이제 각 부분의 중심 내용을 알아보러 가볼까?" 라고 Step 2로 넘어가도록 유도해.`;
                    }
                }

                const step1Socratic = buildSocraticStrictForPartCount(strictCount, repeatNotice);

                let step1Core = '';
                if (useMacroPhase) {
                    step1Core = `[최우선 지침: 교사 데이터 강제 준수]\n이번 답변에서는 **큰 범주** 기준으로 지문이 정확히 ${macroCount}개의 의미 덩어리로 나뉜다고 전제하세요. "나눌 수 없다"거나 "하나다"라고 하는 것은 오답입니다.\n세부 행은 총 ${detailCount}개이나, 지금(1~2단계)은 큰 범주 라벨만 기준으로 유도하고 작은 범주 이름(예: 설명1)은 학생에게 직접 말하지 마세요.\n\n[교사 데이터 - 논리 흐름 요약 (이번 유도: 큰 범주만)]\n큰 범주 수: ${macroCount}\n큰 범주 구조: ${macroLabels.map(l => `[${l}]`).join(' -> ')}\n\n[이 질문의 범위 제한]\n★ 부분 수·라벨은 위 큰 범주만 기준으로 말하세요.\n★ 작은 범주 라벨과 중심 내용(content)은 이번 답변에서 언급하지 마세요.\n`;
                } else {
                    step1Core = `[최우선 지침: 교사 데이터 강제 준수]\n이 지문은 교사 데이터에 따라 정확히 ${detailCount}부분(세부)으로 나뉩니다. "나눌 수 없다"거나 "하나다"라고 하는 것은 오답입니다. 반드시 이 구조를 인정하고 유도하세요.${macroLayerActive ? `\n(참고: 앞 단계에서는 큰 범주 ${macroCount}덩어리로 유도했으며, 지금부터는 반드시 세부 ${detailCount}부분 기준입니다.)` : ''}\n\n[교사 데이터 - 논리 흐름 요약 (세부)]\n부분 수: ${detailCount}\n구조(작은 범주가 있으면 작은 범주, 없으면 큰 범주): ${logicFlowParts.map(s => `[${s.role?.trim() || s.macroLabel?.trim() || '미지정'}]`).join(' -> ')}\n\n[이 질문의 범위 제한 - 절대 준수]\n★ 이 질문(Step 1)은 '부분 수'와 각 행 식별 라벨(작은 범주 우선, 없으면 큰 범주)만 다루는 단계입니다.\n★ 각 부분의 '중심 내용(content)'은 다음 질문(Step 2)에서 다루므로, 이 답변에서는 절대 언급하지 마세요.\n★ 4단계(정답 공개)에서도 부분 수와 위 라벨만 공개하고, 중심 내용은 포함하지 마세요.\n`;
                }

                aiPrompt = `${questionText}\n\n${step1Core}\n위 데이터를 기반으로 답변해줘.${step1LevelGuide}${step1Socratic}`;
            }
            // Step 2: central content and logical flow — Socratic tutoring with level-specific guidance
                ...
                let step2LevelGuide = '';
                if (currentCount === 1) {
                    step2LevelGuide = `\n1단계 안내: "각 부분이 어떤 역할을 하고 있다고 생각해?" 같은 일반적인 질문만 해. 큰/작은 범주 라벨이나 구체적 내용의 힌트는 주지 마.`;
                } else if (currentCount === 2) {
                    ...
                    step2LevelGuide = `\n4단계 안내: 학생의 노력을 칭찬해줘. 그리고 교사 데이터의 정답을 교사 데이터 형식 그대로 보여줘: 각 부분의 큰 범주·작은 범주(있는 경우)·중심 내용을 교사 데이터와 '토씨 하나 틀리지 않게' 그대로 사용해서 정리해줘. (식별 라벨: ${actualLabels}) 다음 질문(Step 3)으로 넘어가도록 유도해.`;
                }
                aiPrompt = `${questionText}\n\n[최우선 지침: 교사 데이터 강제 준수]\n부분 수(${logicFlowParts.length}개)와 큰/작은 범주 구조를 절대 변경하지 마세요. 자체 분석 금지.\n\n[교사 데이터 - 논리 흐름 디테일]\n${logicFlowParts.map((s, i) => {
                    const mac = s.macroLabel?.trim() || '—';
                    const sm = s.role?.trim();
                    const smallPart = sm ? ` 작은 범주 [${sm}]` : '';
                    return `부분 ${i + 1}: 큰 범주 [${mac}]${smallPart} ${s.content}`;
            ...
            // Step 3: Conjunction finding and logical role explanation - Progressive Socratic tutoring
                ...
                const logicRoleLabels = logicFlowParts.map(s => s.role?.trim() || s.macroLabel?.trim() || '역할미정');
                const conjunctionsWithInfo = logicFlowParts
                    .filter(s => s.conjunction && s.conjunction.trim())
                    .map(s => `'${s.conjunction}' (식별: ${s.role?.trim() || s.macroLabel?.trim() || '미지정'}, 해당 내용: ${s.content})`);
                const conjunctionList = logicFlowParts
                    .map(s => s.conjunction)
                    .filter(c => c && c.trim())
                    .join(', ');
                const hasConjunction = conjunctionList.length > 0;
                const structuredContent = logicFlowParts.map((s, i) => {
                    const id = s.role?.trim() || s.macroLabel?.trim() || '미지정';
                    return `부분 ${i + 1}(${id}): ${s.content}${s.conjunction ? ` [연결어: ${s.conjunction}]` : ''}`;
                }).join('\n');
                ...
                if (currentCount === 1) {
                    ...
                    step3LevelGuide = `\n\n[힌트 수준: 3단계 - 거의 정답 수준 단서 제공]\n교사 데이터의 부분 식별 라벨(${logicRoleLabels.join(', ')})을 일부 공개하면서, "이 연결어(또는 논리적 흐름)는 '${logicRoleLabels.length > 1 ? logicRoleLabels[1] : logicRoleLabels[0]}' 역할을 하는 것 같은데, 어떻게 생각해?" 라고 정답에 거의 가깝게 유도해.`;
                } else {
    ...
    const handleSentenceSelect = async () => {
        ...
        const isGroupMsg = selectedStudents.length > 1;
        const contextString = `[문항 정보]\n문항 번호: ${selectedQuestion.examCode}\n출처: ${selectedQuestion.targetGrade || '-'}\n지문: ${selectedQuestion.passage}`;
    ...
    const handleQuestionConfirm = () => {
        if (tempQuestion) {
            ...
            setMessages([{
                role: 'assistant',
                content: `문항 번호 "${tempQuestion.examCode}", 출처 "${tempQuestion.targetGrade || '-'}" 문항이 선택되었습니다.`
            }]);
            ...
            <div className="sc-desktop-layout">
                ...
                <div className="sc-sidebar-panel" style={{ padding: '12px', gap: '12px' }}>
                    ...
                    <div className="sc-card sc-card-pink" style={{ opacity: isStudentSelected ? 1 : 0.5, pointerEvents: isStudentSelected ? 'auto' : 'none', padding: '16px' }}>
                        ...
                        <div className="sc-question-display" style={{ fontSize: '12px', lineHeight: 1.5 }}>
                            {selectedQuestion ? (
                                <>
                                    <div>문항 번호: {selectedQuestion.examCode}</div>
                                    <div>출처: {selectedQuestion.targetGrade || '-'}</div>
                                </>
                            ) : '(선택 안됨)'}
                        </div>
                    </div>
                        ...
                        <div className="sc-modal-box" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="sc-modal-body">
                                {roomQuestions.map(q => (
                                    ...
                                    >
                                        <div style={{ fontWeight: 'bold', color: '#2563eb', fontSize: '16px' }}>문항 번호: {q.examCode}</div>
                                        <div style={{ fontSize: '13px', color: '#64748b', marginTop: '6px' }}>출처: {q.targetGrade || '-'}</div>
                                    </div>
                                ))}
                            </div>
                            <div className="sc-modal-footer">
